...
const GeoLocation: React.FC<GeoLocationProps> = ({ onNext }) => {
    ...
    return (
      ...
      >
        ...
        <motion.select id={"fill-city"} key={"fill-city"} defaultValue=""
        ...
        >
            <option key="sec1" value="" disabled hidden>konum seç</option>
            {Object.keys(location).map( val => 
              <option key={"option-"+val} className={'box m-1.5 p-4 w-full shadow-md text-center font-mono text-stone-100 text-lg backdrop-blur-md bg-stone-800 rounded-md opacity-80 select-none'} value={val}>{val}</option>
            )}
        </motion.select>
        
        { location[city]?
        <motion.select id={"fill-region"} key={"fill-region"} defaultValue=""
          whileHover={{ backgroundColor: 'rgb(68 64 60)', color: 'rgb(250 250 249)' }}
          whileFocus={{ backgroundColor: 'rgb(68 64 60)', color: 'rgb(250 250 249)' }}
          whileTap={{ opacity: 0.5 }}
          initial={{ opacity: 0, y: 20 }}
          ...
          className={'box m-1.5 p-4 w-full shadow-md text-center font-mono text-stone-100 text-lg backdrop-blur-md bg-stone-800 rounded-md opacity-80 select-none'}
        >
            <option key="sec2" value="" disabled hidden>konum seç</option>
            {location[city].map( val =>
              <motion.option key={"option-"+val} className={'box m-1.5 p-4 w-full shadow-md text-center font-mono text-stone-100 text-lg backdrop-blur-md bg-stone-800 rounded-md opacity-80 select-none'} value={val}>{val}</motion.option>
            )}
        </motion.select>:null
        }
        {region!=""?
        <motion.button key={"fill-submit"}
          onTap={ () => {
            if ( region!="" ) onNext({"province": city, "region": region})
          }}
          whileHover={{ backgroundColor: 'rgb(68 64 60)', color: 'rgb(250 250 249)' }}
          whileTap={{ opacity: 0.5 }}
          initial={{ opacity: 0, y: 20 }}
